	...
	while(strcmp(again,"yes") == 0){
		if(*pNrOfPatients<MAX){
			...
			for(int i=0;i<*pNrOfPatients;i++){
				if(strcmp(patientDatabase[i].idNum, idNum)==0){
					printf("The ID-number is not unique, please try again\n");
					printf("Enter ID-number: ");
					gets(idNum);
				} 
			}
			...
			for(int i=0;i<MAXPICTURES;i++){
				printf("Enter image references (max 10): ");
				scanf("%d", &picRef[i]);
				if(picRef[i]==0){
					break;
				}
...
void sortByName(Patient patientDatabase[], int *pNrOfPatients){
	...
	for(int i=0;i<*pNrOfPatients-1;i++){
		for(int j=0;j<*pNrOfPatients-1-i;j++){
...
int searchSelect(Patient patientDatabase[], int *pNrOfPatients){
	...
	int patientFound;
	printf("Search for name(n), id number(i) or image reference(p)? ");
	scanf(" %c", &choice);
	switch(choice){
...
void searchPicRef(Patient patientDatabase[], int *pNrOfPatients, int *pNrOfPatientsFound, int *pPatientFound){
	int searchNum;
	printf("Enter a image reference: ");
	scanf("%d", &searchNum);
	printMenu();
	for(int i=0;i<*pNrOfPatients;i++){
		for(int j=0;j<patientDatabase[i].nrOfPics;j++)
